...
import br.com.projetosar.entidades.Atribuicao
import br.com.projetosar.entidades.UsuarioAtribuicao
import br.com.projetosar.entidades.OAuthID
...
class SpringSecurityOAuthController {
    ...
    def onSuccess = {
        ...
        if (oAuthToken.principal instanceof GrailsUser) {
            ...
            def redirectUrl = "/error_security"
            
            log.fatal "Redirecting to askToLinkOrCreateAccountUri: ${redirectUrl}"
    ...
    def linkAccount = { OAuthLinkAccountCommand command ->
        ...
        if (request.post) {
            boolean linked = command.validate() && Usuario.withTransaction { status ->
                Usuario user = Usuario.findByUsernameAndPassword(
    ...
    def createAccount = { OAuthCreateAccountCommand command ->
        ...
        if (request.post) {
            if (!springSecurityService.loggedIn) {
                ...
                boolean created = command.validate() && Usuario.withTransaction { status ->
                    Usuario user = new Usuario(username: command.username, password: command.password1, enabled: true)
                    user.addToOAuthIDs(provider: oAuthToken.providerName, accessToken: oAuthToken.socialId, user: user)
                    ...
                    for (roleName in config.oauth.registration.roleNames) {
                        UsuarioAtribuicao.create user, Atribuicao.findByAuthority(roleName)
                    }
    ...
    protected OAuthToken updateOAuthToken(OAuthToken oAuthToken, Usuario user) {
        ...
        def userGroupAuthorities = []
        for(grupo in userAuthorities){
            for (grupoAtribuicao in grupo.atribuicoes){
                userGroupAuthorities.add(grupoAtribuicao.atribuicao.authority)
            }
        }
        def authorities = userGroupAuthorities.collect { new GrantedAuthorityImpl(it) }
...
/*
    private def updateUser(Usuario user, OAuthToken oAuthToken) {
        if (!user.validate()) {
            return
        }
    ...
    static constraints = {
        username blank: false, validator: { String username, command ->
            Usuario.withNewSession { session ->
                if (username && Usuario.countByUsername(username)) {
